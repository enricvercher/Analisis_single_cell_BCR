...
Si queremos guardar para analizar en excel:
...
SE PROCESA PARALELAMENTE EL scRNAseq en 03_scRNA_BCRseq_cluster_dobletes-nf, nos quedamos con los LB que sólo expresan BCR de esta librería, quitamos dobletes, clusterizamos etc.

Una vez se ha hecho, seguimos con el análisis del repertorio.
...
for (mouse in names(ratones_clonales)) {
  ...
  table <- mouse_data %>%
    ...
    reframe(
      ...
      CDR3H = dplyr::first(bcr_aa_H[!is.na(bcr_aa_H)]),  # Usar dplyr::first
      CDR3K = dplyr::first(bcr_aa_K[!is.na(bcr_aa_K)]),
      CDR3L = dplyr::first(bcr_aa_L[!is.na(bcr_aa_L)])
    ) %>%
    distinct() %>%
